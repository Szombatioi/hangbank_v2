...
function SectionHeader({ label }: { label: string }) {
  return (
    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 2 }}>
      <Box
        sx={{ width: 4, height: 20, bgcolor: ORANGE, borderRadius: "2px" }}
      />
      <Typography
        sx={{
          fontFamily: LABEL,
...
export default function ViewRecording() {
  ...
  const handleTranscript = useCallback(
    (_takeId: number, text: string, _isFinal: boolean) => setTranscription(text),
    [],
  );
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const { data } = await api.get<AudioFileDetail>(
          `/project/audio-file/${aid}`
        );
        if (cancelled) return;
        setAudioFile(data);
        ...
        if (data.microphoneLabel) {
          const { deviceId: resolved } = await resolveMicrophone(
            data.microphoneLabel
          );
          if (cancelled) return;
    ...
    const loadMasterUrl = async () => {
      try {
        const { data } = await api.get<{ url: string }>(
          `/project/audio-file/${masterId}/url`
        );
  ...
  const refreshQualities = async () => {
    try {
      const { data } = await api.get<AudioQualityMeasure[]>(
        `/audio-quality/audio-file/${aid}`
      );
      setQualities(data);
    } catch {
      /* ignore */
  ...
  const handleSaveRecording = async () => {
    ...
    try {
      ...
      showMessage(
        translateHttpError(err, t, t("view_recording.save_error")),
        Severity.error
      );
    } finally {
      setSaving(false);
  ...
  const handleSaveTranscription = async () => {
    ...
    try {
      await api.patch(`/project/audio-file/${aid}/transcription`, {
        transcription,
      });
    ...
    >
          ...
          useTranscription={
            audioFile.languageCode
              ? {
                  ...
                  transcriptionLang: audioFile.languageCode,
                }
              : undefined
          }
        />
            ...
            <Button
              ...
              sx={{
                ...
                "&:hover": {
                  ...
                  bgcolor: "var(--app-surface-subtle)",
                },
              }}
            >
      ...
      >
        ...
        >
          ...
          <InfoRow
            label={t("view_recording.label_project")}
            value={audioFile.projectName}
          />
          <InfoRow
            label={t("view_recording.label_block")}
            value={
              audioFile.blockIndex != null
                ? `#${audioFile.blockIndex + 1}`
                : undefined
            }
          />
          <InfoRow
            label={t("view_recording.label_name")}
          ...
          <InfoRow
            ...
            value={
              audioFile.durationSeconds != null
          ...
          <InfoRow
            ...
            value={
              audioFile.samplingRate
                ? `${audioFile.samplingRate.toLocaleString()} Hz`
                : undefined
            }
          />
          <InfoRow
        ...
        {audioFile.masterRecording && (
          ...
          >
            ...
            <InfoRow
              label={t("view_recording.label_name")}
              value={audioFile.masterRecording.name}
            />
            <InfoRow
              label={t("view_recording.label_format")}
              value={audioFile.masterRecording.name
            ...
            <InfoRow
              label={t("view_recording.label_length")}
              value={
                audioFile.masterRecording.durationSeconds != null
                  ? formatDuration(audioFile.masterRecording.durationSeconds)
                  : undefined
              }
            />
            {masterUrl ? (
